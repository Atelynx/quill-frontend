export const fieldGroup = 'grid gap-[0.45rem]'

export const formGrid = 'grid gap-4'

export const authGrid = 'grid min-h-screen gap-6 p-6 max-[820px]:grid-cols-1 md:grid-cols-[minmax(0,_1.2fr)_minmax(360px,_0.8fr)] max-[720px]:p-4'

export const authTopbar = 'md:col-span-2 flex items-center justify-between max-[820px]:flex-col max-[820px]:items-stretch max-[820px]:gap-4'

export const authHero = 'text-white p-10 flex flex-col justify-between gap-6 max-[720px]:p-6'

export const authCard = 'p-8 flex flex-col justify-center max-[720px]:p-6'

export const authTabs = 'grid grid-cols-2 gap-3'

export const authTabButton = 'min-h-[48px] px-4 py-3.5 rounded-full border border-[var(--main-page-border)] text-[var(--main-page-text-soft)] bg-[var(--gradient-tab-inactive)] transition-all duration-[var(--main-page-transition)]'

export const authTabActive = 'bg-[var(--gradient-tab-active)] text-white border-[color-mix(in_srgb,_var(--color-accent)_24%,_transparent)]'

export const authStatusStack = 'min-h-[12px] mt-4'

export const heroMetrics = 'grid gap-3.5'

export const heroMetricCard = 'p-4 bg-[var(--gradient-metric-surface)] border border-[var(--main-page-inverse-border)] rounded-[var(--main-page-radius-md)]'

export const loadingScreen = 'min-h-screen grid place-items-center text-[var(--main-page-text-soft)]'

export const buyModeToggle = 'flex gap-2'

export const buyModeButton = 'flex-1 min-h-[42px] px-4 py-2.5 text-[0.92rem]'
