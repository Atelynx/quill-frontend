...
import { hint, textSoft, textMuted } from '../../../shared/design-system/typography';
...
export function ConfigHistoryView({ keyName, onClose }: ConfigHistoryViewProps) {
  ...
  return (
    <div className={surface.modalOverlay} onClick={onClose}>
      <div className={`rounded-[var(--main-page-radius-lg)] shadow-[var(--main-page-shadow)] border border-[var(--main-page-border)] w-full max-w-2xl ${surface.strong} p-6 max-h-[80vh] flex flex-col`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto flex-1">
          {isLoading ? (
            ...
            <div className="grid gap-3">
              {history.map((entry) => (
                <div key={entry._id} className={surface.innerCard}>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <span className="font-semibold text-text">{String(entry.value)}</span>
                      {entry.name ? (
                        <p className={`m-0 text-[0.85rem] ${textSoft}`}>{entry.name}</p>
                      ) : null}
                    </div>
                    <div className={`text-right text-[0.82rem] ${textMuted}`}>
                  ...
                  {entry.tags?.length ? (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {entry.tags.map((tag) => (
                        <span key={tag} className={`rounded-full bg-[var(--main-page-surface-muted)] px-2.5 py-0.5 text-[0.78rem] ${textSoft}`}>
                          {tag}
                        </span>
                      ))}
                    </div>
